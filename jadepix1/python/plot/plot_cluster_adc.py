...
def plot(fname,title):
    ...
    cluster_th2f = ROOT.TH2F('CHIP_%s_ClUSTER_ADC'%title,'CHIP_%s_CLUSTER_ADC'%title,25,0,25,500,0,5000)
    ...
    size_th2f = ROOT.TH2F('CHIP_%s_SIZE'%title,'CHIP_%s_SIZE'%title,500,0,5000,25,0,25)
...
def main():
    ...
    output = ROOT.TFile('../output/Cluster_ADC.root','recreate')

    for chip_address in xrange(1,7):
        cluster_th2f,size_th2f = plot('../output/CHIPA%d_Cluster55_Iron55_thr500.root'%chip_address,'A%d'%chip_address)
