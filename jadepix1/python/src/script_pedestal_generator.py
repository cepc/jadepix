...
def generate(chip_address,title,input,output):
    ...
    template_file = open(r'./python/src/template_pedestal_script.py','r')
    tmpl = Template(template_file.read())
    lines = []
    lines.append(tmpl.substitute(INPUT = input,\
                                 OUTPUT= output))
    if not os.path.exists('./python/src/src_%s_pedestal/'%chip_address):
        os.makedirs('./python/src/src_%s_pedestal/'%chip_address)
    python_file_name = './python/src/src_%s_pedestal/%s.py'%(chip_address,title)
...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 2:
        if sys.argv[1].startswith('-a'):
            ...
            if not os.path.exists('./python/output/output_%s_pedestal/'%chip_address.lower()):
                os.makedirs('./python/output/output_%s_pedestal/'%chip_address.lower())
                
            for input_dir in source_data_dir[chip_address.lower()]:
                run(chip_address,input_dir,'./python/output/output_%s_pedestal/'%chip_address.lower())
