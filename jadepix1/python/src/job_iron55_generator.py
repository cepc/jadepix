...
def generate(chip_address,title):
    ...
    src = './python/src/src_%s_iron55/%s.py'%(chip_address,title)

    if not os.path.exists('./python/run/jobs_%s_iron55/'%chip_address):
        os.makedirs('./python/run/jobs_%s_iron55/'%chip_address)

    lines = []
    lines.append(tmpl.substitute(SRC = src))

    job_file_name = './python/run/jobs_%s_iron55/Job_%s'%(chip_address,title)
