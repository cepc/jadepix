...
def generate(title):
    ...
    src = './python/src/chip_a1/%s.py'%title

    if not os.path.exists('./python/run/jobs_chip_a1/'):
        os.makedirs('./python/run/jobs_chip_a1/')

    lines = []
    lines.append(tmpl.substitute(SRC = src))

    job_file_name = './python/run/jobs_chip_a1/Job_%s'%title
