...
def generate(title):
    ...
    src = './python/src/src_a1_iron55/%s.py'%title

    if not os.path.exists('./python/run/jobs_a1_iron55/'):
        os.makedirs('./python/run/jobs_a1_iron55/')

    lines = []
    lines.append(tmpl.substitute(SRC = src))

    job_file_name = './python/run/jobs_a1_iron55/Job_%s'%title
...
if __name__ == '__main__':
    main('/publicfs/atlas/atlasnew/higgs/tth/hbb/raw_pixel/20180329/')