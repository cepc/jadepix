...
def generate(chip_address,title):
    ...
    src = './python/src/src_%s_sr90/%s.py'%(chip_address,title)

    lines = []
    lines.append(tmpl.substitute(SRC = src))

    if not os.path.exists('./python/run/jobs_%s_sr90/'%chip_address):
        os.makedirs('./python/run/jobs_%s_sr90/'%chip_address)

    job_file_name = './python/run/jobs_%s_sr90/Job_%s'%(chip_address,title)
