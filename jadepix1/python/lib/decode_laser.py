...
'''
MOUDLE : decode source data to cluster with excluding overlap signal
'''
...
class Decode():
    ...
    def __init__(self,input,output):
        ...
        self.cf.read('./conf/python_configure.conf')

        self.print_number = int(self.cf.get('laser_decode','print_number'))
        self.try_process_number = int(self.cf.get('laser_decode','try_process_number'))
        self.max_frame_number = int(self.cf.get('laser_decode','max_frame_number'))
        self.laser_threshold = int(self.cf.get('laser_decode','laser_threshold'))
        self.binary_threshold = int(self.cf.get('laser_decode','binary_threshold'))
        self.cluster_conf = int(self.cf.get('laser_decode','cluster_conf'))

        #init ROOT value

        self.seed_signal = np.zeros(1, dtype='int32')
        self.cluster_signal = np.zeros(1, dtype='int32')

        self.cds_mass_center_list = np.zeros(1, dtype='float32')
        self.cds_mass_center_x_list = np.zeros(1, dtype='float32')
        self.cds_mass_center_y_list = np.zeros(1, dtype='float32')

        self.binary_mass_center_list = np.zeros(1, dtype='float32')
        self.binary_mass_center_x_list = np.zeros(1, dtype='float32')
        self.binary_mass_center_y_list = np.zeros(1, dtype='float32')
        ...
        self.laser_tree = ROOT.TTree('Laser_Tree','Laser')

        self.laser_tree.Branch('Seed_Signal',self.seed_signal,'Seed_Signal/I')
        self.laser_tree.Branch('Cluster_Signal',self.cluster_signal,'Cluster_Signal/I')
        self.laser_tree.Branch('CDS_Mass_Center_Distribution',self.cds_mass_center_list,'CDS_Mass_Center_Distribution/F')
        self.laser_tree.Branch('CDS_Mass_Center_X_Distribution',self.cds_mass_center_x_list,'CDS_Mass_Center_X_Distribution/F')
        self.laser_tree.Branch('CDS_Mass_Center_Y_Distribution',self.cds_mass_center_y_list,'CDS_Mass_Center_Y_Distribution/F')

        self.laser_tree.Branch('Binary_Mass_Center_Distribution',self.binary_mass_center_list,'Binary_Mass_Center_Distribution/F')
        self.laser_tree.Branch('Binary_Mass_Center_X_Distribution',self.binary_mass_center_x_list,'Binary_Mass_Center_X_Distribution/F')
        self.laser_tree.Branch('Binary_Mass_Center_Y_Distribution',self.binary_mass_center_y_list,'Binary_Mass_Center_Y_Distribution/F')
    ...
    def get_laser_frame(self,cds_frame_adc):
        ...
        max_pos = np.where(cds_frame_adc == max_adc)
        max_pos_x = max_pos[0][0]
        max_pos_y = max_pos[1][0]
        ...
        if max_adc > self.laser_threshold :
            ...
            logging.debug('\n\n******* signal frame *******\n\n')
            logging.debug(str(laser_cds_frame))

            laser_binary_frame = np.where(cds_frame_adc > self.binary_threshold, 1, 0)
            logging.debug('\n\n******* binary frame *******\n\n')
            logging.debug(str(laser_binary_frame))

            self.get_current_mass_center(laser_cds_frame,laser_binary_frame,max_pos_x,max_pos_y)
    ...
    def get_current_mass_center(self,laser_cds_frame,laser_binary_frame,max_pos_x,max_pos_y):
        
        cds_mass_center_numerator = 0.
        binary_mass_center_numerator = 0.

        x_pos_start = max_pos_x - self.cluster_conf
        x_pos_end = max_pos_x + self.cluster_conf

        y_pos_start = max_pos_y - self.cluster_conf
        y_pos_end = max_pos_y + self.cluster_conf 

        cluster_size = 2 * self.cluster_conf + 1
        cluster_cds_array = []

        # dangerous ! #
        for x_pos in xrange(x_pos_start,x_pos_end+1):
            for y_pos in xrange(y_pos_start,y_pos_end+1):
                cluster_cds_array.append(laser_cds_frame[x_pos][y_pos])

        self.seed_signal[0] = laser_cds_frame[max_pos_x][max_pos_y]
        self.cluster_signal[0] = np.sum(cluster_cds_array)


        cds_mass_center_xy = ndimage.measurements.center_of_mass(laser_cds_frame)
        binary_mass_center_xy = ndimage.measurements.center_of_mass(laser_binary_frame)
        logging.info('cds center xy:'+str(cds_mass_center_xy))
        logging.info('binary center xy:'+str(binary_mass_center_xy))

        for i in xrange(48):
            for j in xrange(16):
                cds_mass_center_numerator +=  float(laser_cds_frame[i][j]*math.hypot(i,j))
                binary_mass_center_numerator +=  float(laser_binary_frame[i][j]*math.hypot(i,j))
        cds_mass_center_denominator = np.sum(laser_cds_frame)
        binary_mass_center_denominator = np.sum(laser_binary_frame)

        cds_mass_center = cds_mass_center_numerator/cds_mass_center_denominator
        binary_mass_center = binary_mass_center_numerator/binary_mass_center_denominator

        self.cds_mass_center_list[0] = cds_mass_center
        self.cds_mass_center_x_list[0] = cds_mass_center_xy[0]
        self.cds_mass_center_y_list[0] = cds_mass_center_xy[1]

        self.binary_mass_center_list[0] = binary_mass_center
        self.binary_mass_center_x_list[0] = binary_mass_center_xy[0]
        self.binary_mass_center_y_list[0] = binary_mass_center_xy[1]

        self.laser_tree.Fill()
    ...
    def process_frame(self):
        ...
        self.laser_tree.GetCurrentFile().Write()
        self.output.Close()
        data.close()
